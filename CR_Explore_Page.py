...
def show_explore_page():
    ...
    @st.cache(max_entries = 10, ttl = 3600)
    def make_fig3():
        fig3 = px.imshow(new_df[filter].corr().round(2), text_auto = True)
        fig3.update_layout(title_text = '<b>Correlation Matrix</b>', title_x = 0.5)
        fig3.update_xaxes(tickangle = 90)
        fig3.layout.height = 600
        return fig3
    
    # Show Correlation Matrix
    cm.plotly_chart(make_fig3(), use_container_width = True) 
    ...
    left, pcd, right = st.columns([5, 50, 5])
    ...
    def make_fig5():
        ...
        fig5.update_layout(title_text = '<b>Parallel Category Diagram</b>', title_x = 0.5)
        return fig5  
